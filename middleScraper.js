...
app.get('/', function(req, res){
    ...
    request({uri: url, jar: true}, function(error, response, html){
        ...
        if(!error){
            ...
            var iFrame = $('#iFrame_edlElement_8845358563349194884_document');
            ...
            request({uri: src, jar: true}, function(error, response, html){
                ...
                for (var i=0; i < schoolsArray.length; i++) {
                    ...
                    if (schoolName !== "")
                    {
                        ...
                        middleSchools.push(schoolToAdd);
                    }
                }
                fs.writeFile('middleSchools.json', JSON.stringify(middleSchools, null, 4), function(err){
